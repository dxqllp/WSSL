...
class CocoDetection(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        if self._transforms is not None:
            img, target = self._transforms(img, target)#进行标准化处理
...
def generate_target_training(target, K=1):
    ...
    relative_x = torch.Tensor(N, K).uniform_(-0.5 + eps, 0.5 - eps)#uniform_从均匀分布中抽取数值进行填充
    relative_y = torch.Tensor(N, K).uniform_(-0.5 + eps, 0.5 - eps)

    x = boxes[:, 0, None] + boxes[:, 2, None] * relative_x#none 关键词是在后面增加一个维度
...
def build(image_set, args):
    ...
    mode = 'instances'
    PATHS = {
        "train": (root / "minitrain2017", root / "annotations" / f'mini{mode}_train2017_with_points.json'),
        "val": (root / "minival2017", root / "annotations" / f'mini{mode}_val2017_with_points.json'),
    }
